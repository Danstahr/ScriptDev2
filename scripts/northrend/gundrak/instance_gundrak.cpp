...
struct MANGOS_DLL_DECL instance_gundrak : public ScriptedInstance
{
    ...
    void HandleGameObject(const uint64& GUID, bool open, GameObject *go = NULL)
    {
        if (!go)
   ...
   bool IsEncounterInProgress() const
    ...
    void OnCreatureCreate(Creature* pCreature)
    {
        switch(pCreature->GetEntry())
        {
            ...
            case 29573: uiDrakkariElemental = pCreature->GetGUID(); break;
        }
    }
    ...
    uint64 GetData64(uint32 type)
    {
        switch(type)
        {
            ...
            case DATA_DRAKKARI_ELEMENTAL:         return uiDrakkariElemental;
        }
...
bool GOHello_altar(Player *pPlayer, GameObject *pGO)
{
    ...
    if (instance_gundrak* pInstance = (instance_gundrak*)pGO->GetInstanceData())
    {
        uint64 uiStatue;
        switch(pGO->GetEntry())
        {
        ...
        pInstance->DoUseDoorOrButton(uiStatue);
        pInstance->CheckAltars();
        return true;
    }
